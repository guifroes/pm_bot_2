...
3.  **Estratégia de Teste:** O testes de comportamento devem ser o mais realista possível, de preferência sem usar mocks, acessando os endpoints reais. exceto quando há risco de usar dinheiro.
...
Analise a especificação na pasta `specs` Se uma feature parecer complexa, **quebre-a agressivamente**.

As fatias não devem seguir a divisnao de arquivos das specs e sim casos de uso do sistema. Por exemplo: uma fatia deve ser o sign in na polymarket, o que pode tocar varias specs.

Create `IMPLEMENTATION_PLAN.md` usando este formato para o output:
